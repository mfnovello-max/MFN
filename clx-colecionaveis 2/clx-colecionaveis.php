...
final class CLX_Colecionaveis {
    ...
    public function render_shortcode(): string {
        ...
        // Seu CSS do arquivo usa seletor mais forte (#clx-page-colecionaveis ...),
        // então aqui eu já coloco o MESMO prefixo pra garantir que o ajuste "pegue".
        wp_add_inline_style('clx-colecionaveis', <<<CSS
/* ===== Ajustes finos do card (override) ===== */
...
#clx-page-colecionaveis .clx-card-body{
  padding: .44rem .30rem .52rem;
  flex: 1 1 auto;
  display:flex;
  flex-direction:column;
  min-height: 220px; /* ajuste aqui */
}

/* ✅ Espaço entre PREÇO e mini-info (o que você pediu) */
#clx-page-colecionaveis .clx-price{
  margin: 0 0 .55rem !important; /* ajuste aqui (ex: .45rem / .65rem) */
}
        ...
        <div id="clx-page-colecionaveis" class="clx-page" data-clx-root>
            ...
            <div class="clx-layout">
                <aside class="clx-sidebar" data-clx-sidebar>
                    <div class="clx-sidebar-scroll">
<div class="clx-sidebar-head">
                        <h2>
                            <span class="clx-meta-bullet clx-meta-bullet--filter" aria-hidden="true">
                                <img src="https://mfn.cobraleds.com.br/wp-content/uploads/2026/02/icone-semente-com-filtro-final-v2.svg" alt="" loading="eager" decoding="async" />
                            </span>
                            <span class="clx-sidebar-title-text">
                                <span><?php esc_html_e('Encontre', 'clx-colecionaveis'); ?></span>
                                <span><?php esc_html_e('sua semente', 'clx-colecionaveis'); ?></span>
                            </span>
                        </h2>
    ...
    private function render_filters(array $filters): string {
        ...
        <form class="clx-filters" data-clx-form>
            ...
            <div class="clx-filter-actions">
                <button type="button" class="clx-btn clx-btn--secondary" data-clx-reset><?php esc_html_e('Limpar filtros', 'clx-colecionaveis'); ?></button>
            </div>
        </form>
        <?php
        return (string) ob_get_clean();
    ...
    private function render_active_filters(array $filters): string {
        ...
        foreach ($labels as $key => $label) {
            if (!empty($filters[$key])) {
                $chips[] = sprintf(
                    '<button type="button" class="clx-chip" data-clx-remove="%s">%s: %s</button>',
                    esc_attr($key),
                    esc_html($label),
                    esc_html($filters[$key])
                );
            }
        }
        ...
        if ($filters['min_price'] !== '' || $filters['max_price'] !== '') {
            $chips[] = sprintf(
                '<button type="button" class="clx-chip" data-clx-remove="price">%s: %s - %s</button>',
                esc_html__('Preço', 'clx-colecionaveis'),
                esc_html($filters['min_price'] ?: '0'),
                esc_html($filters['max_price'] ?: '∞')
            );
        }
        ...
        // Chip extra para limpar tudo (mesmo comportamento do botão Limpar)
        $chips[] = '<button type="button" class="clx-chip clx-chip--clear" data-clx-clear-filters>' . esc_html__('Limpar filtros', 'clx-colecionaveis') . '</button>';
